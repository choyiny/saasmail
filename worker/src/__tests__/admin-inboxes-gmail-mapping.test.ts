...
describe("admin inboxes router — Gmail mapping", () => {
  ...
  it("verifies send-as for a body that names only gmailAccountId, and marks the row gmail", async () => {
    // The gate was entered only when `source` was in the request, so this
    // body — the one field that creates the mapping — skipped it entirely
    // and wrote an unverified row that the sync then fed, while the UI
    // rendered it as "Cloudflare".
    const { apiKey } = await createTestUser({ role: "admin" });
    await seedGmailAccount();
    const fetchMock = stubSendAs({ addresses: ["nope@elsewhere.dev"] });
  ...
  it("writes source gmail when a body names only gmailAccountId and passes", async () => {
    const { apiKey } = await createTestUser({ role: "admin" });
    await seedGmailAccount();
    ...
    expect(res.status).toBe(200);
    const rows = await rowsFor("a@x.com");
    // Naming a mailbox is choosing Gmail: the row the sync reads and the row
    // the send path reads must be the same row.
    expect(rows[0].source).toBe("gmail");
    expect(rows[0].gmailAccountId).toBe("acct-1");
  });
