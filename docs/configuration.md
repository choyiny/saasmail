...
- `TOKEN_ENCRYPTION_KEY` — Does two jobs for the [Gmail integration](gmail.md): it encrypts stored Gmail refresh tokens, and it is the HMAC secret that signs the OAuth `state` parameter carried through Google's consent screen. 32 random bytes, base64-encoded (`openssl rand -base64 32`). Set in prod via `wrangler secret put`. Never rotate this without reconnecting every mailbox.
