...
- **Gmail integration: the credential layer.** An admin can connect a Google Workspace mailbox to saasmail over OAuth, list the connected mailboxes, and disconnect one, through three new routes — `GET /api/admin/gmail/connect`, `GET /api/admin/gmail`, and `DELETE /api/admin/gmail/{id}` — plus the `GET /api/admin/gmail/callback` Google redirects to. The refresh token is sealed with AES-GCM under a new `TOKEN_ENCRYPTION_KEY` secret before it reaches the database, and the list route never returns token material. This exists because a domain has one set of MX records: a root domain already on Google Workspace can never reach Cloudflare Email Routing, so saasmail has to read that mailbox through the Gmail API instead of competing for delivery. **No mail syncs yet** — this release stores credentials and nothing else, and no admin UI ships with it; message sync, sending, and mapping a Gmail mailbox to a saasmail inbox come later. Requires `GOOGLE_OAUTH_CLIENT_ID`, `GOOGLE_OAUTH_CLIENT_SECRET`, and `TOKEN_ENCRYPTION_KEY`; without all three the routes answer `503`. The consent screen must be **Internal**, which limits this to an operator who owns the Workspace they are connecting. Migration `0035`. See [Gmail integration](docs/gmail.md).
