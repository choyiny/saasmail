...
- **`GET /api/inboxes` — the addresses the caller may send from.** Admins get every inbox; members get the ones assigned to them. Until now the only inbox list was `GET /api/admin/inboxes`, which is admin-only, so a member had no way to discover a `fromAddress` that `POST /api/send` would accept: a client could either hardcode one, or offer a picker built by guessing and let the user find out via 403. The list is derived from the same universe the admin route builds and narrowed by the same permission resolver the send route enforces with, so what a picker shows is exactly what will send. Carries display name, display mode and signature; deliberately omits `forwardTo` and `assignedUserIds`, which are admin configuration and a disclosure of who else holds an inbox.
