...
- **A Gmail-mapped inbox whose Google grant cannot produce a token now refuses the reply instead of rerouting it.** It used to fall back to the configured provider, which was dishonest in both directions: on a Gmail-only install there is no provider, so the outbox marked the row failed while the API answered `201` and the composer cleared the draft — nothing sent, the user told it was; on an install with Resend or Postmark the reply left a real Workspace address through a service that domain's SPF/DKIM does not authorise, failing DMARC and retrying every 15 minutes. The reply is now refused with a `502`, nothing is written and nothing is queued, and the composer keeps the draft. An inbox that is **not** Gmail-mapped — including every inbox when the OAuth secrets are unset — still uses the configured provider exactly as before. **If you were relying on the old fallback, this is a behaviour change.** See [A broken grant refuses the reply](docs/gmail.md#a-broken-grant-refuses-the-reply-it-does-not-reroute-it).
- A Gmail `2xx` that carries no message id is now treated as **delivered** rather than as a transient failure, and the error text says so instead of advising a resend — Gmail accepted the message, so sending it again would deliver the customer a second copy. What is lost is saasmail's own `sent_emails` row; the Sent-folder mirror puts the message back on the timeline.
- Remapping an inbox to a different Google mailbox, unmapping it, or disconnecting a mailbox now **clears the Gmail thread ids** stored for those inboxes. Thread ids are per-mailbox, so a stale one handed to a different account's send endpoint is a `4xx` — terminal, never queued — which made every conversation predating the change permanently unreplyable. Replies on those conversations now start a new Gmail thread; the saasmail timeline, which groups by person and conversation, does not move.
- The send-as verification on `PATCH /api/admin/inboxes/{email}` keys on the mapping the request would **result** in, not on which fields it carried, so a body naming only `gmailAccountId` can no longer write a Gmail mapping without the check. A body combining `"source": "cloudflare"` with a `gmailAccountId` is refused with a `400` before Google is contacted.
- The OAuth consent flow is **bound to the admin who started it**. The signed `state` names that admin and the callback requires the session finishing it to be the same one; a flow completed in somebody else's browser is refused with `?gmail=wrong_admin`, before the authorization code is spent.
- The connected-mailbox list shows who added each mailbox and when — "Added 3 days ago · last connected by Jane Ops" — and `GET /api/admin/gmail` returns `connectedBy`. It is null on a mailbox connected before the column existed, and its `name`/`email` are null if that admin has since been deleted.
- A sync run that fails before syncing anything now records `lastError` on the account: `sync_failed:http_500`, `sync_failed:rate_limited`, `sync_failed:unknown`, or a bare auth code. A mailbox that had stopped syncing used to look healthy. **Reconnect is offered only for `sync_failed:http_401` and `sync_failed:http_403`** — the credential being refused is the only one a fresh grant fixes; every other code withholds the button, because a consent round trip would re-seed the cursor and lose mail to fix nothing. Neither the cursor nor `lastSyncedAt` moves.

- **API failures show the server's explanation instead of a status code.** The web app used to surface `API error: 502` for anything that failed, which tells a user nothing about what to do; it now shows the message the server sent — why a mapping was refused, why a reply could not be sent, which address needs verifying — and falls back to the bare status only when the response carries no message of its own.
...
- The public invite-acceptance and first-run setup pages no longer show an unauthenticated visitor the raw text of an internal failure. Both forwarded whatever `auth.api.createUser` threw; they now forward only better-auth's own `APIError` message — the text written for the person filling in the form, such as "User already exists" — and answer anything else with a generic "please try again" while the real reason goes to the Worker log. A database error's message names the statement it was running, and on that path its bound parameters are the credentials being registered.
...
- A reply mirrored from a Gmail Sent folder can no longer be written twice by two overlapping cron ticks. Migration `0041` makes `sent_emails (gmail_message_id, from_address)` unique. **Upgrade caveat:** SQLite refuses to build a unique index over data that already violates it, so an instance that ran an earlier build of the Gmail sending feature must clear any duplicate pair before this migration applies — the check query is in [Setup](docs/gmail.md#setup).
- The admin **Inboxes** page can load the connected-mailbox list at all. The client requested `/api/admin/gmail/` with a trailing slash, which the worker does not route: in production the SPA catch-all answered it with `index.html` and a `200`, so parsing the body threw. The list always read "Couldn't load the connected mailboxes", which also disabled every **Source** dropdown and locked every mapped row — the whole Gmail admin UI was unreachable.
...
- A failed Gmail reply's `502` no longer carries the text of the token re-authorization that failed — it classifies to a code and logs that instead. That frame ends in a D1 statement binding the plaintext access token, and a D1 error carries the parameters bound to it.

- **Every timestamp in the connected-mailbox list.** "Synced 5 minutes ago" and the **Sync gap** notice read the unix seconds the worker stores as milliseconds, so every healthy mailbox rendered as "Synced 20698 days ago" and every sync gap as ancient history — inverting the one signal each line exists to give. Only the admin **Inboxes** page was affected.
- **Cloudflare Email Sending carries every `Cc` recipient.** It kept only the last one: the header was set once per address, and each call replaced the previous, so a message Cc'd to three people reached one of them with no error anywhere. This affects every install sending through Cloudflare Email Sending, Gmail or not — replies, forwards, and any send with more than one `Cc`. Resend, Postmark and Bavimail were never affected.
...
- **An admin UI for Gmail mailboxes.** Connecting a mailbox and mapping an inbox to it no longer need hand-made API calls. The **Inboxes** page gains a **Connected Google mailboxes** section — connect a mailbox, see when each one last synced, reconnect one whose grant broke, and disconnect one behind an inline confirm — and every inbox row gains a **Source** dropdown that points it at Cloudflare Email Routing or at one of the connected mailboxes. Returning from Google's consent screen shows whether the connection worked; a failure says only that it failed, because the underlying error can carry the access token. A mailbox that has stopped syncing prints the reason it stopped, and offers **Reconnect** only for the errors a fresh grant can fix — `no_personal_inbox`, `ambiguous_inbox_mapping`, `history_gap` and `message_failed:<id>` mean the mapping or the mail is wrong, not the grant, so the button is hidden and the note says what to do instead. **Reconnect targets the mailbox it belongs to:** that address is sent to Google as a `login_hint` and sealed into the signed OAuth state, and if a different Google account is granted anyway the callback writes nothing and says which mailbox you meant versus which one you gave — previously it would have replaced a different mailbox's credentials and reset its sync cursor. **A reconnect now records a sync gap**, because re-seeding the cursor at the mailbox's current position permanently skips whatever arrived while the grant was dead; like every sync gap it never clears, since reconnecting cannot un-lose that mail. Disconnecting a mailbox unmaps the inboxes that read from it rather than leaving them pointing at a mailbox that is gone. A row whose mapping cannot be matched now distinguishes "Mailbox no longer connected" from "Couldn't check" when the mailbox list itself failed to load, and **any failure to load that list locks every mapped row** — a row the last good list can still name keeps showing its address rather than an opaque id, but changing it is refused until the list is reloaded, so a network blip cannot talk you into unmapping a working inbox. Mapping an inbox is still verified against what the connected account can actually send from, and the UI now shows you which of the three refusals you hit and what it needs: a `400` (the account cannot send as that address — add it under **Send mail as**, and note that an alias awaiting verification is refused too, since Gmail will not send from one), a `502` (the check could not be completed — retry), or a `503` (Gmail integration is not configured — set the OAuth secrets). `PATCH /api/admin/inboxes/{email}` also rejects `"source": "gmail"` with no `gmailAccountId` with a `400`. Connecting still does **not** backfill, and mail still takes up to 15 minutes to appear. See [Gmail integration](docs/gmail.md).
- **Gmail mailbox sync.** An admin can connect a Google Workspace mailbox to saasmail over OAuth, map it to a saasmail inbox, and its mail starts landing on that inbox's timeline — through `GET /api/admin/gmail/connect`, `GET /api/admin/gmail`, `DELETE /api/admin/gmail/{id}`, the `GET /api/admin/gmail/callback` Google redirects to, and `PATCH /api/admin/inboxes/{email}` for the mapping. This exists because a domain has one set of MX records: a root domain already on Google Workspace can never reach Cloudflare Email Routing, so saasmail has to read that mailbox through the Gmail API instead of competing for delivery. A cron tick polls every connected mailbox every 15 minutes, so **a message can take up to 15 minutes to appear**. Connecting a mailbox does **not backfill** — the timeline starts empty and fills going forward only, never importing mail that was already there. **Only personal Gmail mailboxes are supported**, one mailbox per inbox; Google Group addresses are rejected with a `400`, since routing a shared mailbox's mail by message headers turned out to be forgeable by the sender and was withdrawn rather than shipped. A mapping is checked before it is saved against the addresses the connected account can actually send from — its own address plus each "Send mail as" alias whose verification has completed — so a mapping the account could not honour is rejected with a `400` at mapping time instead of failing later on a real reply; an alias that is listed but still awaiting verification is refused too, because Gmail will not send from one. If Gmail cannot be reached to run that check, the mapping is refused with a `502` rather than saved unverified. Read state isn't written back to Gmail in either direction — that's later work. A revoked grant stops that mailbox syncing until it's reconnected, and if a mailbox goes unpolled for about a week, Gmail expires its history cursor and re-seeds from the present — mail from inside that gap cannot be recovered; the new `lastGapAt` field on `GET /api/admin/gmail` records when that last happened. The refresh token is sealed with AES-GCM under `TOKEN_ENCRYPTION_KEY`, and the list route never returns token material. Requires `GOOGLE_OAUTH_CLIENT_ID`, `GOOGLE_OAUTH_CLIENT_SECRET`, and `TOKEN_ENCRYPTION_KEY`; without all three the routes answer `503`. The consent screen must be **Internal**, which limits this to an operator who owns the Workspace they are connecting. Migrations `0035`, `0036`, `0037`. See [Gmail integration](docs/gmail.md), and [Gmail on a local instance](docs/gmail-local-setup.md) to set it up locally.
- **Gmail sending, and a two-way Sent mirror.** A reply typed in saasmail to a Gmail-mapped inbox (`POST /api/send/reply/{emailId}`) now sends through that mailbox's real Gmail account instead of the configured provider — landing in its own Sent folder, threaded onto the Gmail conversation. And the sync in the other direction: a reply typed directly in Gmail is mirrored from that mailbox's Sent folder onto the customer timeline within the next poll (up to 15 minutes), which is the consolidation this whole feature exists for. **Bulk and campaign mail still goes out through the configured provider, on purpose** — new compose, template sends, and sequence steps never touch Gmail, even for a Gmail-mapped `fromAddress`, because Google Workspace caps sending at roughly 2,000 messages/day per user and Gmail API sends count against it; routing bulk mail through Gmail would burn a real person's quota and risk their account. **A failed Gmail reply is not retried** — saasmail returns a `502` and does not queue it, because the retry queue would resend through the configured provider under a different identity, with different DKIM, missing from the Sent folder and with threading broken; send it again by hand, unless the error asks you to reconnect the mailbox. If the OAuth secrets are unset no inbox is Gmail-mapped at all, so replies go through the configured provider as they always did. But an inbox that **is** Gmail-mapped and cannot produce a token — a revoked grant, or Google unreachable — has its reply **refused** with the `502` above rather than rerouted; the account's `lastError` records the failed token refresh. (Unset secrets write no `lastError`: saasmail never reaches the account row to write one.) saasmail's own replies coming back through the Sent mirror are recognized and skipped, so nothing appears twice. Migrations `0039`, `0040`. See [Sending through Gmail](docs/gmail.md#sending-through-gmail).
