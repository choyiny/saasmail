...
- **The admin surface is reachable over an OAuth token, drawn by damage rather than by category.** The bearer deny list refused nine operations for looking like they rewrote the deployment, while `DELETE /api/admin/users/{id}` and `DELETE /api/blocklist/mail` — more destructive than any of them — were open at `admin:manage` the whole time. One question decides it now: does the route create a privilege that survives revoking the client and expiring the token, or open a standing copy of future mail? Revoking an invite, sending a webhook test, listing and revoking OAuth clients, and replacing an inbox's assignments are open. Four more are reachable only in a shape that cannot escalate: a token-minted invite must be `role: "member"`, must name an email address, and expires within a week; a role change may only demote; an inbox's `forwardTo` and the webhook URL may be cleared but not set, so an integration gets a kill switch it can never use to arm. That last pair is the exfiltration case — the webhook payload carries subject and body text, so a URL set from a token copies every future inbound message to it without the token ever holding `email:read`. The body clamps are allowlists: a field added to one of those routes later is refused to token callers until someone classifies it, the same way an unclassified path is. Closed outright are `/api/api-keys`, `/api/user/passkeys` and `/api/auth/*` — an `sk_` key skips the scope policy entirely, never expires, and is not revoked with the client — and `GET /api/admin/invites`, which returns the tokens of live invites, including admin-role ones a human created in a browser with no email pinned. Session and API-key callers are unaffected by all of it.
