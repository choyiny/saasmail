...
- **Domain health.** `GET /api/domains` (admin-only) lists every domain the deployment handles mail for, with the number of inboxes under it, the number of messages it has received, and a live DNS status. The list is derived from the addresses that already exist — `DISTINCT emails.recipient` ∪ `sender_identities.email`, split at the `@` and lower-cased — so there is no domains table and no migration. The DNS check reads MX and TXT over Cloudflare's DNS-over-HTTPS resolver and reports whether mail routes to Cloudflare (`*.mx.cloudflare.net`), somewhere else, or nowhere, and whether an SPF record exists and includes `_spf.mx.cloudflare.net`; whatever is still missing comes back as `{name, type, value}` records for the client to render and copy. A resolver that cannot be reached answers `unknown` rather than a verdict — a timeout is not a misconfigured domain — and emits no missing records. Answers are cached for 60 seconds so a screen refresh does not re-query.
