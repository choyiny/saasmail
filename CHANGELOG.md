...
- **Domain health.** `GET /api/domains` (admin-only) lists every domain the deployment handles mail for, with the number of inboxes under it, the number of messages it has received, and a live DNS status. The list is derived from the addresses that already exist — `DISTINCT emails.recipient` ∪ `sender_identities.email`, split at the `@` and lower-cased — so there is no domains table and no migration. The DNS check reads MX, SPF, and the Email Routing DKIM key over Cloudflare's DNS-over-HTTPS resolver and reports whether mail routes to Cloudflare (`*.mx.cloudflare.net`), somewhere else, or nowhere; whether an SPF record exists and includes `_spf.mx.cloudflare.net`; and whether the routing DKIM record is published at `cf2024-1._domainkey.<domain>`. Whatever is still missing comes back as `{name, type, value, action, note}` records for the client to render and copy. `action` is `replace` where a record already exists at that name and must be edited rather than duplicated — an existing SPF record arrives already merged with the Cloudflare `include`, because only one SPF record per name is legal and adding a second breaks outbound mail. The DKIM record has a `null` value: Cloudflare generates that key per domain, so the operator copies it from Email Routing → Settings in the dashboard. A resolver that cannot be reached answers `unknown` rather than a verdict — a timeout is not a misconfigured domain — and a check that answered `unknown` contributes no missing records of its own; the checks resolve independently, so the others still report. Answers are cached for 60 seconds so a screen refresh does not re-query.
