...
- **Native push notifications via Expo.** Web Push is predicated on a browser `PushSubscription` — a push-service URL plus the P-256 keypair the `PushManager` generated — which React Native cannot produce: it has no ServiceWorker and no `PushManager`, only an opaque `ExponentPushToken`. Delivery now branches by transport, with the Web Push path unchanged. Registrations live in a new `expo_push_subscriptions` table rather than sharing the existing one, which would have required making `p256dh` and `auth` nullable — and SQLite cannot drop a NOT NULL constraint in place, so that means a create-copy-drop-rename of a live table carrying an index and a foreign key. Migration `0033` is purely additive.
- **Notification previews are off by default, and configurable.** A native payload passes through Expo and then Apple or Google before reaching a lock screen, so by default a notification says only that mail arrived and carries opaque routing ids; the client fetches the content over an authenticated connection. Set `PUSH_PREVIEWS="true"` to include the sender and subject. Web Push is encrypted end-to-end to the browser and keeps its existing payload.
- **VAPID is no longer required for deployments that only serve native clients.** The VAPID guard sat at the top of the delivery path, so an unset key skipped every transport rather than just Web Push. It now guards only the branch that needs it.
- `POST` and `DELETE /api/notifications/expo/subscribe` register and remove a device, keyed on a client-supplied installation id rather than the token. Expo rotates tokens — on reinstall, on restore to a new handset — and keying on the token would accumulate a dead row per rotation, each of which still looks live until something tries to send to it. Tokens are stored server-side and never returned by any route.
- **`GET /api/config` advertises an API version and capability flags.** The web UI ships from the same commit as the worker and never needs to ask what it supports; a third-party or native client talks to whatever version an operator is running, and previously could only discover a missing capability by attempting it and interpreting the failure — indistinguishable from a bug, a misconfiguration or a revoked grant. `apiVersion` plus `capabilities.oauthApi` and `capabilities.oauthStream` let a client say "this server needs a newer saasmail" before it starts a flow it cannot finish. Purely additive; existing consumers read the fields they name and ignore the rest.
- **The realtime notification stream accepts OAuth bearer callers.** `GET /api/notifications/stream` required an `Origin` header matching `TRUSTED_ORIGINS`, which a native client cannot satisfy — it has no browser origin to send. That check defends against Cross-Site WebSocket Hijacking, an attack that works because a browser attaches the session cookie to a cross-origin handshake automatically; a bearer token is not ambient, so no attacker page can cause a victim's token to be attached to a socket it opened. The check now follows the credential it protects: unchanged for session cookies, skipped for OAuth. API keys keep their existing behaviour.
- **`/api/*` accepts OAuth 2.1 access tokens.** The deployment already issued them, but only `/mcp` accepted them — `/api/*` string-matched the `Bearer sk_` API-key prefix and rejected everything else, so a third-party or native client could be granted scopes it had no way to use. Tokens are now a third credential alongside the session cookie and API keys, resolved through the same validator `/mcp` uses so client revocation, ban and passkey rules cannot drift between the two surfaces.
- **Scopes bound what an OAuth client may do.** Session and API-key callers are unchanged and remain unscoped. Token callers are checked per route against an explicit method-plus-path policy, and anything unclassified is refused, so a route added later cannot silently widen every existing token. Three routes send mail from under a router whose other routes do not — template send, sequence enroll and outbox retry — and each requires `email:send` rather than the `email:manage` a prefix-based rule would have inferred. The account and credential surface (`/api/api-keys`, `/api/user/passkeys`, `/api/auth/*`) is closed to tokens entirely: minting an API key would convert a narrow mail consent into the user's whole surface and destroy their existing key.
- New `admin:manage` scope gating the admin surface for OAuth callers. Never implied, and held _in addition to_ `role === "admin"` — a consented scope must not promote a member.
- `GET /api/user/me` returning the caller's id, email, name and role, so a client can decide whether to offer admin screens without probing an admin route and reading the 403.
- `Authorization` and `Content-Type` are declared in the CORS allow-list. Hono's default is empty, so a cross-origin client that preflighted a request carrying a bearer token had the header stripped and the request blocked.
